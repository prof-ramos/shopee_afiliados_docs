...
class ShopeeAffiliateClient:
    ...
    def generate_short_link(
        ...
        """
        # subIds (utm_content): documentação permite até 5.
        # Na prática a API pode rejeitar certos formatos (ex.: underscore). Usar somente [A-Za-z0-9].
        if sub_ids:
            if len(sub_ids) > 5:
                raise ValueError("sub_ids deve ter no máximo 5 itens")
            invalid = [s for s in sub_ids if (not isinstance(s, str) or not s.isalnum())]
            if invalid:
                raise ValueError(
