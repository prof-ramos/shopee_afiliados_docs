...
class SubIdTester:
    ...
    def __init__(self):
        ...
        if not self.app_id or not self.app_secret:
            raise ValueError(
                "SHOPEE_APP_ID and SHOPEE_APP_SECRET must be set in environment variables"
            )

        self.client = ShopeeAffiliateClient(self.app_id, self.app_secret)
        self.results = []

    def test_subid_format(
        self, sub_ids: List[str], test_name: str, expected: str = "unknown"
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            response = self.client.generate_short_link(
                origin_url=test_url, sub_ids=sub_ids
            )
    ...
    def run_all_tests(self):
        ...
        self.test_subid_format(
            ["s1", "s2", "s3", "s4", "s5"], "Simple letters with numbers", "valid"
        )
        ...
        self.test_subid_format(
            ["promo1", "promo2", "sale1"], "Words with numbers", "valid"
        )
        ...
        self.test_subid_format(
            ["sub_1", "test_2"], "Underscore with numbers", "invalid"
        )
        ...
        self.test_subid_format(
            ["test1test2test3test4test5"], "Long concatenated word", "unknown"
        )
